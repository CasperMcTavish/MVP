...
def main():
    array = np.loadtxt("phiarray2D.txt")
    ...
    slice[slice==0] = 0.001
    negative_slice[negative_slice==0] = 0.001
    log_nslice = np.log(negative_slice)
    log_slice = np.log(slice)
    pos_list[pos_list==0] = 0.001
    ...
    # LOG LOG
    plt.figure(figsize=(5,5))
    plt.scatter(log_list,log_slice[int(len(log_slice)/2)])
    plt.scatter(log_list,log_nslice[int(len(log_nslice)/2)])
    plt.title("CLEAN - logR, logV")
    plt.show()
