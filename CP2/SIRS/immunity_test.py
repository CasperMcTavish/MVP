...
def immune():
    ...
    i_frac_list = []
    i_error_list = []
    # immunity list from 0.1 to 0.6
    imm_list = np.linspace(0.1,0.7,num=13, endpoint=True)
    ...
    for j in range(len(imm_list)):
        ...
        for i in range(5):
            ...
            i_frac_temp.append(inf_mean)

        # Calculate mean and error for each point, then append to frac list
        i_frac_list.append(np.mean(i_frac_temp))
        i_error_list.append(sem(i_frac_temp))
